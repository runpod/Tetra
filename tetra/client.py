...
class ResourceManager:
    ...
    async def get_or_create_resource(self, config: Dict[str, Any], resource_type: str) -> str:
...
class RunPodServerlessStub:
    ...
    def __init__(self, endpoint_url):
        ...
        print(f"Initialized RunPod stub for endpoint: {endpoint_url} (ID: {self.endpoint_id})")
    ...
    async def ExecuteFunction(self, request):
        ...
        payload = {
            ...
            "kwargs": {k: v for k, v in request.kwargs.items()}  # Convert to regular dict
        }
        ...
        try:
            ...
            run_request = await loop.run_in_executor(
                None, 
                lambda: self.endpoint.run({"input": payload})
            )
            ...
            if isinstance(output, dict) and "success" in output:
                ...
                serialized_result = base64.b64encode(cloudpickle.dumps(output)).decode('utf-8')
                return remote_execution_pb2.FunctionResponse(
                    success=True,
            ...
            return remote_execution_pb2.FunctionResponse(
                success=False,
                error=f"RunPod request failed: {str(e)}"
            )
...
def remote(server_spec: Union[str, List[str]] = None, fallback: Union[None, str, List[str]] = None, 
           resource_config: Dict[str, Any] = None, resource_type: str = None,
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if resource_config and resource_type:
                ...
                try:
                    ...
                    if server_name not in global_client.servers:
                        ...
                        if not resource_id:
                            raise ValueError(f"Resource details not found for {server_name}")
                        ...
                        if resource_details["type"] == "serverless":
                            ...
                            print(f"Registering RunPod endpoint: {server_name} at {endpoint_url}")
                            ...
                            raise ValueError(f"Unsupported resource type: {resource_details['type']}")
                ...
                if isinstance(server_spec, dict) and type is not None:
                    ...
                    if type == "serverless":
                        await global_client.add_runpod_server(temp_server_name, endpoint_url)
                    else:
            ...
            serialized_args = [base64.b64encode(cloudpickle.dumps(arg)).decode('utf-8') for arg in args]
            serialized_kwargs = {k: base64.b64encode(cloudpickle.dumps(v)).decode('utf-8') for k, v in kwargs.items()}
